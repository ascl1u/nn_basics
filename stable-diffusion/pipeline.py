...
def generate(prompt: str, uncond_prompt: str, input_image=None, strength=0.8,
    ...
    with torch.no_grad():
        ...
        if do_cfg:
            ...
            cond_context = clip(cond_tokens) # (batch_size, seq_len, dim)
            ...
            context = torch.cat([cond_context, uncond_context]) # (2, seq_len, dim)
        else:
            tokens = tokenizer.batch_encode_plus([prompt], padding="max_length", max_length=77).input_ids
            tokens = torch.tensor(cond_tokens, dtype=torch.long, device=device) # (batch_size, seq_len)
            context = clip(tokens) # (1, batch_size, seq_len)
        to_idle(clip)
        ...
        if input_image:
            ...
            input_image_tensor = input_image_tensor.unsqueeze(0) # (batch_size, height, width, channel)
            input_image_tensor = input_image_tensor.permute(0, 3, 1, 2) # (batch_size, channel, height, width)
            ...
            latents = torch.randn(latents_shape, generator=generator, device=device) # start with random noise N(0, I)
        diffusion = models["diffusion"]
        diffusion.to(device)
        timesteps = tqdm(sampler.timesteps)
        for i, timestep in enumerate(timesteps):
            time_embd = get_time_embedding(timestep).to(device) # (1, 320)
            model_input = latents # (batch_size, 4, latents_height, latents_width)
            if do_cfg:
                model_input = model_input.repeat(2, 1, 1, 1) # (2 * batch_size, 4, latents_height, latents_width)
            model_output = diffusion(model_input, context, time_embd) # predicted noise by the UNET
            if do_cfg:
                output_cond, output_uncond = model_output.chunk(2)
                model_output = cfg_scale * (output_cond - output_uncond) + output_uncond
            latents = sampler.step(timestep, latents, model_output) # remove noise predicted by UNET
        to_idle(diffusion)
