...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x, context):
        ...
        x = self.conv_input(x)
        n, c, h, w = x.shape
        x = x.view((n, c, h * w)) # (batch_size, features, height * width)
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        #x: (batch_size, features, height, width)
        x = F.interpolate(x, scale_factor = 2, mode="nearest") # (batch_size, features, height * 2, width * 2)
        return self.conv(x)
...
class UNET(nn.Module):
    def __init__(self):
        ...
        self.decoders = nn.ModuleList([
            SwitchSequential(UNET_ResidualBlock(2560, 1280)), # (batch_size, 1280, height / 64, width / 64)
...
class UNET_OutputLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.silu(x)
        x = self.conv(x)
        return x # (batch_size, height / 8, width / 8)

class Diffusion(nn.Module):
    def __init__(self):
        self.time_embd = TimeEmbedding(320)
        self.unet = UNET()
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        ...
        # context: (batch_size, seq_len, dim)
        # time: (1, 320)
        time = self.time_embd(time) # (1, 1280)
